...
export function CampBulkImportModal({
  isOpen,
  onClose,
  onImportComplete,
  campId,
  ...
  const downloadTemplate = () => {
    ...
    const sampleData = [
      'Jean,Tremblay,jean@email.com,514-555-0123,2010-05-15,forward,99,Pierre Tremblay,514-555-0124,pierre@email.com,Marie Tremblay 514-555-0125,No allergies'
    ];
    ...
    a.href = url;
    a.download = `${campName.toLowerCase().replace(/\s+/g, '_')}_players_template.csv`;
    a.click();
  ...
  const handleMappingConfirm = () => {
    if (state.mappingResult?.requiredMissing.length) {
      setState(prev => ({
        ...prev,
        errors: [`Required fields missing: ${state.mappingResult?.requiredMissing.join(', ')}`]
      }));
      return;
    }
    ...
    try {
        ...
        for (const playerData of batch) {
          try {
            ...
            if (existingPlayer) {
              ...
              results.playersExisting++;
              
              // Convert to frontend format for later use
            ...
            // Now register the player to the camp (whether new or existing)
            ...
            if (existingRegistration) {
              ...
              if (registrationError) {
                results.errors.push({
                  player: `${playerData.first_name} ${playerData.last_name}`,
                  error: `Player ${playerCreated ? 'created' : 'found'} but registration failed: ${registrationError.message}`
                });
              } else {
      ...
      setState(prev => ({
        ...
        importResults: {
          ...
          playersExisting: results.playersExisting,
          registrationsCreated: results.registrationsCreated,
          registrationsExisting: results.registrationsExisting,
          errors: [
            ...results.errors,
            {
              player: 'General Import',
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {state.mappingResult.unmappedFields.length > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Unmapped fields</h4>
                    {state.mappingResult.unmappedFields.map((field, index) => (
                      <div key={index} className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg mb-2">
                        ...
                        <span className="text-yellow-700 ml-2 text-sm">(will be ignored)</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {state.step === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start">
                  <UserPlus className="w-5 h-5 text-blue-500 mt-0.5 mr-3" />
                  <div>
                    <h4 className="text-sm font-medium text-blue-800 mb-1">
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                {state.mappedPlayers.length > 10 && (
                  <div className="px-4 py-3 bg-gray-50 text-sm text-gray-500 text-center">
                    ... and {state.mappedPlayers.length - 10} more players
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <Button onClick={handleImport} className="bg-red-600 hover:bg-red-700">
                  ...
                  Import {state.mappedPlayers.length} player(s)
                </Button>
              </div>
            </div>
          )}
          ...
          {state.step === 'importing' && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                Please wait while we import your players and register them to the camp
              </p>
            </div>
          )}
          ...
          {state.step === 'results' && state.importResults && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                ) : state.importResults.registrationsCreated > 0 ? (
                ...
                <p className="text-gray-600">
                  {state.importResults.registrationsCreated > 0 
                    ? `${state.importResults.registrationsCreated} player(s) registered to camp successfully`
                    : "No players could be registered to the camp"
                  }
                </p>
              </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <ul className="text-sm text-green-700 space-y-1">
                  <li>• {state.importResults.playersCreated} new players created</li>
                  <li>• {state.importResults.playersExisting} existing players found</li>
                  <li>• {state.importResults.registrationsCreated} new camp registrations</li>
                  <li>• {state.importResults.registrationsExisting} players already in camp</li>
                  <li>• {state.importResults.errors.length} errors</li>
                </ul>
              </div>
